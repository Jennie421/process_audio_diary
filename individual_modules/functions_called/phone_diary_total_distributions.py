...
def study_dists(study):
	...
	if not transcript_qc_dist.empty:
		...
		# distribution_plots(transcript_qc_dist, pdf_out_path_trans, ignore_list=["day","OLID","hours_until_submission"], 
	...
	# 	except:
	# 		pass
	# 	distribution_plots(OS_dist, pdf_out_path_OS, ignore_list=["day","OLID","hours_until_submission"], bins_list=[24,10,12,20,10,10,16,10,10,6,10,10,25,25,30,30,10,10,16,7,16,7,24,12,16,8,12,6,24,10,12,6,32,16,12,6],
	...
	if not transcript_nlp_dist.empty:
		try:
			...
			pass
		# distribution_plots(transcript_nlp_dist, pdf_out_path_nlp, ignore_list=["day","OLID","hours_until_submission"],
...
def study_summary(study):
	...
	combined_df = pd.read_csv(study_wide_metadata_loc)
	feature_combined_df = combined_df[diary_ID_feats + audio_qc_feats + transcript_qc_feats + nlp_feats]
	feature_combined_df.to_csv(combined_dist_path,index=False)

	# NOTE actual plotting. 
	# try:
	...
	# except:
	# 	pass
	# distribution_plots(combined_df, combined_pdf_path, ignore_list=["day","OLID"])
