...
def transcript_pull(study, OLID, password, pipeline=False, lab_email_path=None):
	...
	directory = os.path.join("$study_loc", study, OLID, "phone/processed/audio/pending_audio")
	...
	for filename in cur_pending:
		...
		local_path = os.path.join("$study_loc", study, OLID, "phone/processed/audio/$transcripts_loc", transname)
